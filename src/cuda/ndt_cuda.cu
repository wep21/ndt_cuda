...
#include <ndt_cuda/cuda/covariance_regularization.cuh>
#include <ndt_cuda/cuda/find_voxel_correspondences.cuh>
#include <ndt_cuda/cuda/ndt_compute_derivatives.cuh>
...
NDTCudaCore::NDTCudaCore() {
  ...
  distance_mode = ndt_cuda::NDTDistanceMode::D2D;
  set_neighbor_search_method(ndt_cuda::NeighborSearchMethod::DIRECT7, 0.0);
}

NDTCudaCore::~NDTCudaCore() {}

void NDTCudaCore::set_distance_mode(ndt_cuda::NDTDistanceMode mode) {
...
void NDTCudaCore::set_neighbor_search_method(ndt_cuda::NeighborSearchMethod method, double radius) {
  ...
  switch (method) {
    ...
    case ndt_cuda::NeighborSearchMethod::DIRECT1:
      h_offsets.resize(1);
      h_offsets[0] = Eigen::Vector3i::Zero();
      break;

    case ndt_cuda::NeighborSearchMethod::DIRECT7:
      h_offsets.resize(7);
    ...
    case ndt_cuda::NeighborSearchMethod::DIRECT27:
    ...
    case ndt_cuda::NeighborSearchMethod::DIRECT_RADIUS:
...
void NDTCudaCore::create_source_voxelmap() {
  ...
  if (source_voxelmap || distance_mode == ndt_cuda::NDTDistanceMode::P2D) {
    return;
  }

  source_voxelmap.reset(new GaussianVoxelMap(resolution));
  source_voxelmap->create_voxelmap(*source_points);
  covariance_regularization(source_voxelmap->voxel_means, source_voxelmap->voxel_covs, ndt_cuda::RegularizationMethod::MIN_EIG);
}
...
void NDTCudaCore::create_target_voxelmap() {
  ...
  covariance_regularization(target_voxelmap->voxel_means, target_voxelmap->voxel_covs, ndt_cuda::RegularizationMethod::MIN_EIG);
}
